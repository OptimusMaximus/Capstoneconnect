...
@section('content')
    ...
    <?php 
          ...
          $answers = DB::table('answers')->join('evaluations', 'answers.eid', '=', 'evaluations.id')
                        ->where('answered_about', '=', $userid)->get();
          
          //$users = User::all();
          $answered_about = DB::table('users')->where('id', '=', $userid)->get();
          $answered_about_user =  $answered_about[0]->first_name.' '.$answered_about[0]->last_name;
    ?>
    
    <p><b><font size ='6'>{{$answered_about_user}}</font></b></p>
    @if ($answers != null)
    @foreach($answers as $answer)
        ...
        <table class="table table-bordered table-groups pull-right">
                ...
                <p>Evaluated by {{$answered_by_user}}</p>
